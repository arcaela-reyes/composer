...
class LocalBitcoin {
    ...
    public function wallet($then=null, $catch=null){
        return $this->_query("/api/wallet-balance/")->then(function(Object $response) use($then){
            $data = [
                "address"=> $response->data? $response->data['receiving_address'] : 'UNKANOWM',
                "balance"=>  $response->data? (float) $response->data['total']['balance'] : 0,
                "available"=>  $response->data? (float) $response->data['total']['sendable'] : 0,
            ];
